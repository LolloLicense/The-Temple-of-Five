...
function getCurrentPlayerShareText(shareUrl: string): string {
  ...
  if (!currentName) {
    if (highscores.length === 0)
      return `Can you beat The Temple of the Five? Play here: ${shareUrl}`;
  ...
  const mine = highscores.find(
    (h) => h.name.trim().toLowerCase() === normalizedCurrent,
  );
...
export function initHighscoreShare(config: { shareUrl: string }): void {
  ...
  const closeBtn =
    document.querySelector<HTMLButtonElement>("#shareModalClose");
  ...
  window.addEventListener("keydown", (e) => {
    if (!el.modal.classList.contains("hidden") && e.key === "Escape")
      closeModal();
  });
  ...
  el.copyBtn.addEventListener("click", async () => {
    ...
    setStatus(
      ok ? "Copied! Paste it on Instagram / anywhere." : "Could not copy",
    );
  });
