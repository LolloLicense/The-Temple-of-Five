...
export function initAudioManager(configs: SoundConfig[]): void {   // initializes the audio manager by loading all the sound files specified in the soundsConfig array
    ...
    for (const cfg of configs) {   // create new audio-element
...
export async function syncMutedFromToggle(): Promise<void> {    // promise because playBgm is async, and we want to await it if we are unmuting to ensure the bgm starts playing before the function finishes
    ...
    if (!muted) {   // if we are unmuting and there is a desired background music ID, start playing the background music for the current room
...
export function stop(id: string): void {   // stop the currently playing background music immediately without fading
    const audio = sounds.get(id);   // get the audio element for the specified id from the sounds Map
    ...
    if (playingBgmId === id) {   // if the stopped background music is the currently playing one, reset the playingBgmId to null
        playingBgmId = null;
    }
}
...
async function fadeOut(id: string, ms: number): Promise<void> {
    ...
    for (let i = 0; i < steps; i++) {       // loop 20, check volume, check if fade aborted, await stepMs
