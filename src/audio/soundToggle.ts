...
export function toggleMuted(): boolean {
  // toggles the state of muted and returns the new state
...
export function initSoundToggle(): void {
  // initializes the sound toggle button and its function
  ...
  function renderSoundIcons(): void {
    // if muted is true, add "hidden" class to soundIconActive; if muted is false, remove "hidden" class from soundIconActive
  ...
  toggleSoundBtn.addEventListener("click", async () => {
    // when the toggleSoundBtn is clicked, toggle the state of muted and log the new state to the console, then update the sound icons
