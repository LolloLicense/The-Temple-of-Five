# Gruppkontrakt - THE POGO STICK PIONEERS 👹

## Tidigare grupparbeteserfarenheter

Vilka är respektive gruppmedlems top 3 sämsta erfarenheter med grupparbeten? Vilka "negativa förväntningar" kommer ni in i detta med? <br>

Enhetligt beslut i gruppen:

- <b>Person som ej tar sitt ansvar och gör sina uppgifter.</b>
- <b>Tekniska konflikter.</b>
- <b>Otydlig arbetsfördelning / Dålig kommunikation.</b>

> [!TIP]
> Det är bra att få ur sig de negativa förväntningarna och att diskutera öppet kring tidigare erfarenheter. Ju öppnare ni är i er kommunikation nu, desto bättre grupparbete kommer ni att få. På en arbetsplats kanske man känner varandra redan och det finns andra "krav". Av någon anledning beter sig många annorlunda i en skolmiljö när det gäller grupparbeten :thinking:

## Mötestider ⏰

Vilka dagar och tider jobbar vi gemensamt?

- Daily standup _varje vardag_. Det får gärna vara samma tid så att det är lätt att komma ihåg, men det är upp till gruppen att bestämma. Ni ska fylla i en loggbok dagligen med er daily. <br>
  <b>Daily standup: 11.00 Varje dag</b> <br><br>
- Tid för backlog refining varje vecka (cirka 1 h). <br>
  <b>Måndagar 09.15</b> <br><br>
- Tid för sprint planning varje vecka (cirka 1-2 h). <br>
  <b>Måndagar 10.15</b> <br><br>
- Tid för retrospektiv varje vecka (cirka 1 h). <br>
  <b>Fredagar 09.15</b>

## Möteskanaler & format 📽️

- Hur träffas vi? Discord, Teams, på någon fysisk plats… <br>
  <b>Discord / Teams. Beroende på uppgift, skall det delas skärm använder vi Teams.</b> <br> <br>
- Kamera av/på? <br>
  <b>Valfritt, inget krav</b> <br> <br>
- Ska man komma förberedd eller går vi gemensamt igenom vad som ska göras på mötet? <br>
  <b>Vi kommer förberedda till mötet om det är något som skall presenteras. I planeringsfaser går vi tillsammans igenom vad som skall göras.</b> <br> <br>
- Definiera vad som är "brådskande", och hur man kommunicerar om något är brådskande. SMS? Telefonsamtal? <br>
  <b>Discord, kanal som är döpt till just brådskande. Alla medlemmar har discord i telefonen för kontakt.</b> <br><br>
- Befinner vi oss alla i samma tidszon? <br>
  <b>Alla medlemmar är medvetna om att vi träffas på svensk tidzon.</b>

## Kommunikation ☎️

- Via vilka kanaler kommunicerar vi? <br>
  <b>Discord, Teams</b> <br><br>
- Vilka tider kommunicerar vi? <br>
  <b> 09.00 - 17.00 </b> <br><br>
- Inom vilken tidsfrist kan man förvänta sig svar/återkoppling? <br>
  <b> Inom 6h, annars nytt försök. </b> <br><br>
- Hur meddelar man förhinder? I hur god tid meddelar man förhinder? <br>
  <b> Vi meddelar i "planering" i discord om man får förhinder. Vi meddelar så snart som möjligt. Är det nåogt brådskande har vi en kanal som heter "brådskande" där vi meddelar i. </b> <br><br>
- Överväg att dela er mail/telefonnummer så att ni faktiskt kan nå varandra. <br>
  <b> Discord är den enklaste lösning för alla i gruppen </b> <br><br>
- Hur föredrar vi att jobba? Live-koda tillsammans? På egen hand? Ska vi göra kod-merges ihop? <br>
  <b> Beroende på uppgift. Live-codar tillsammans för globala regler. Vi kodar enskilt i separata filer för uppgifter som rör enskild. Merga till "developer-branch" innan commit till main, vilket vi gör tillsammans. </b> <br><br>
- Vem ansvarar för att dokumentera daily standup i repot? (Rotera gärna denna roll med någon frekvens så att inte en person gör det enbart). <br>
  <b> Aktuell scrum-master ansvarar för anteckningar </b> <br><br>

## Personlighetstyper i gruppen :cowboy_hat_face: :partying_face: :exploding_head: :nerd_face: :clown_face:

👸<b>Louise:</b> <br>

Hurdant reagerar varje gruppmedlem vid stress?

- Jag är en kombination av att fly situationen & Försöker få ordning på situtation ( Få alla må bra/ komma överrens.

Vilken roll brukar varje gruppmedlem ta på sig i ett grupparbete? Vilken roll har du "i det tysta" för dig själv?

- Planeraren/Genomföraren: Organiserad, praktisk, pålitlig

---

🦸‍♂️<b>Emil:</b> <br>

Vid stress:

- Försöker få alla att må
  bra / komma överens

Roller:

- Infomationssökaren
- Granskaren/ Utvärderaren
- Och lite Djävulens advokat

---

🧑‍🌾<b>Alex:</b> <br>

Hurdant reagerar varje gruppmedlem vid stress?

- Försöker få alla att må bra/komma överens

Vilken roll brukar varje gruppmedlem ta på sig i ett grupparbete? Vilken roll har du "i det tysta" för dig själv?

- Lagspelaren/Samordnaren: Samarbetsvillig, diplomatisk, stöttande.
- Pepparen: Positiv, empatisk, skapar god stämning.

---

👩‍🚀<b>Minai:</b>

- Under stress blir jag manisk och försöker lösa alla problem på en gång. Mycket känns överväldigat men priot är att alla mår bra, och vi har bra kommunikation.

Roller:

- Pepparen, Planeraren, Slutföraren

---

🧙‍♀️<b>Alexandra:</b>

Hurdant reagerar varje gruppmedlem vid stress?

- Jag reagerar ganska dåligt på stress men det syns inte utåt. Går in i mig själv och börjar jobba väldigt fokuserat, tar på mig förmycket och tar inte pauser glömmer äta, kissa pratar inte med min sambo. Blev så under första javascript uppgiften. Gråter väldigt mycket och delar oftast inte med mig till andra när jag blir det men försöker. Jag kan bli korthuggen och försöker hela tiden gå vidare och inte fastna.

Roll:

- Jag driver gärna men är mer åt planerings hållet än ledar hållet.

---

Hurdant reagerar varje gruppmedlem vid stress?

- Flyr situationen, slutar delta, ignorerar situationen/grupparbetet
- Blir konfrontativ/aggressiv
- Fryser till, vet inte vad man ska göra, svårt att komma igång och/eller vidare
- Försöker få alla att må bra/komma överens
- På något annat sätt?

Vilken roll brukar varje gruppmedlem ta på sig i ett grupparbete? Vilken roll har du "i det tysta" för dig själv?

- **Ledaren:** Drivande, dynamisk, tar initiativ.
- **Planeraren/Genomföraren:** Organiserad, praktisk, pålitlig
- **Experten/Specialisten:** Djup kunskap, fokuserad, analytisk
- **Informationssökaren:** Nyfiken, faktabaserad, samlar data
- **Slutföraren/Kvalitetssäkraren:** Noggrann, detaljfokuserad, avslutar i tid.
- **Innovatören/Idésprutan:** Kreativ, fantasifull, ser nya lösningar.
- **Granskaren/Utvärderaren:** Kritisk, strategisk, objektiv, logisk.
- **Djävulens advokat:** Utmanande, ifrågasätter konstruktivt.
- **Lagspelaren/Samordnaren:** Samarbetsvillig, diplomatisk, stöttande.
- **Pepparen:** Positiv, empatisk, skapar god stämning.
- **Dörrvakten/Processledaren:** Säkerställer deltagande, hanterar diskussionen.

---

## Uppgiftsfördelning

- Hur fördelas uppgifter? <br>
  <b> Genom sprint och poäng, i enhetlighet med grupp och slutligen aktuell scrum-master. </b>

- Hur kommunicerar man när man tar på sig en uppgift utanför daily standupen, eller ska man vänta till standupen? <br>
  <b> Genom discordkanelen planering. </b>

- Vad är definitionen på "klar"? <br>
  <b> När vi har reviewat som grupp i spalten "Review" och flyttat issuen till "Done" samt mergat. </b> <br>

- Hur granskar vi varandras kod?
  <b>Vid behov / support / konflikt, granskar vi koden tillsammans, i mindre eller större grupp. </b>

---

## Övriga förväntningar

- Vilken ambitionsnivå har vi? <br>
  <b> Initialt vill vi nå dess krav och inom tidsramen. Gruppen är enig om att göra ett spel som vi är stolta över och kan visa upp. </b>

- Hur mycket förväntas man delta/hur aktivt ska man vara? <br>
  <b>Precis som på ett jobb. Vi är överens om att vi färdigställer de uppgifter som fördelas till var och en </b>

- Vad blir konsekvenserna om man inte är aktiv i projektet? Hur många kontaktförsök gör man? Hur snabbt ska personen återkomma som inte varit aktiv/gått att nå? Hur fort tar man kontakt med läraren om någon inte dyker upp/gör det man kommit överens om?
  <b> Få man inte kontakt med personen inom 3 dagar så kontaktar vi läraren (Jenni). Nolltollerans för oanmäld frånvaro. </b>

---

- Vad är du bra på?
- Vad är du sämre på?
- Finns det något du rent tekniskt vill träna extra på? HTMl, CSS, git? Projektledning?

---

👸 <b>Louise:</b> <br>

Jag är sämre på att - göra en sak i taget / kasst tålamod.

Bättre på - Figma / design , få saker gjort ( går ihop med mitt tålamod- vill bli klar)

🧙‍♀️ <b>Alexandra:</b> <br>

Bra på: Html och sass/css gillar struktur/design

"Sämre" är jag på javscript och behöver hålla på med det ett längre tag innan man kan säga sig vara bra på de. Behöver mer tid för att koda javascript

🦸‍♂️ <b>Emil:</b> <br>

Bra på: Javascript,html, css

Dålig på att skapa egen grafik

🧑‍🌾 <b>Alex:</b> <br>

Bra på:
Struktur
Planering
"Nöta och sätta sig in i något"

Sämre på:
Skapa grafik
Jobba i högt tempo
git

👩‍🚀 <b>Minai:</b> <br>

Bra: Organiserad, Empantisk, Planering, Stöd.

Dålig: Tar på mig för mycket, Grunderna sitter inte helt 100%

---

## Scrum master schema 👩‍⚖️

<b>Alex</b>: 11 feb - 16 feb <br>
<b>Alexandra</b>: 17 feb - 20 feb <br>
<b>Minai</b>: 23 feb - 26 feb <br>
<b>Louise</b>: 27 feb - 4 mars <br>
<b>Emil</b>: 5 mars - 10 mars <br>

Ny Scrum master väljs ut 10 mars

## Kodstandard 😨

- Hur skriver vi våra commit-meddelanden? Ska vi följa någon särskild standard (t.ex. Conventional Commits eller använda gitmoji).
  <b> Conventional Commits. Tydliga i vår beskrivning </b>
- Hur indenterar vi vår kod?
  <b> Genom Biome, alla sätter samma config </b>
- Vilket språk använder vi i vår kod, för t.ex. CSS-selektorer?
  <b> Vi kommer anvvända engelska. JS, Class, id görs alltid i CamelCase
- Hur kommenterar vi vår kod?

## Övrigt

Om ni vill ta upp någonting utöver det fördefinierade. Skriv gärna en egen rubrik.

## Underskrifter

- [Alexander Johansson](https://github.com/AlexJCodes)

- [Emil Lychnell](https://github.com/elychnell)

- [Louise Sverkström](https://github.com/LolloLicense)

- [Minai Karlsson](https://github.com/minza-42)

- [Alexandra Henriksson](https://github.com/xAlexCode)
